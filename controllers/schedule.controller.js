  ...
  try {
    ...
    if (!project_id || !supervisor_id || !schedule_date) {
      return res
        ...
        .json({
          message:
            "All fields (project_id, supervisor_id, schedule_date) are required.",
        });
    }
    ...
    if (!supervisor) {
      return res
        .status(404)
        .json({ message: `Supervisor with ID ${supervisor_id} not found.` });
    }
...
const getProjectSchedules = async (req, res) => {
  try {
    ...
    if (!schedules.length) {
      return res
        .status(404)
        .json({ message: `No schedules found for project ID ${project_id}.` });
    }
...
const getScheduleDetails = async (req, res) => {
  try {
    ...
    if (!schedule) {
      return res
        .status(404)
        .json({ message: `Schedule with ID ${schedule_id} not found.` });
    }
